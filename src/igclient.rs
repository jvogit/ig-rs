use self::{
    ...
    igrequests::{IGLoggedOutRequestMetadata, IGRequest},
};
use cookie::Cookie;
use reqwest::{
...
impl IGLoggedOutClient {
    ...
    pub fn with_session_cookies(session_cookies: Option<&HeaderValue>) -> IGLoggedInClient {
        let cookie_store = Arc::new(Jar::default());
        cookie_store.set_cookies(
            &mut session_cookies.into_iter(),
...
impl IGLoggedInClient {
    pub fn session_cookies(&self) -> Option<HeaderValue> {
        self.cookie_store
